...
NUMBER_OF_MODELS = 7
IMAGE_SIZE = [112, 112]
BINS_DIR_LOC = '/RG/rg-tal/orlev/project/insightface/datasets'
...
def set_models(models_dir, epochs, batch_size, image_size):
    ...
    for model_loc, epoch in zip(models_dir, epochs):
        ...
        nets.append(model)
        #return nets, models_thresholds, models_thresh_names                     ########################### for more than one models delete it
...
def bin_loc_to_data(bins_dir_loc, image_size):
    bins_test_list, bins_train_list = set_bins_loc(bins_dir_loc)
    ...
    data_dict = {'train': (train_data_list, train_issame_list), 'test': (test_data_list, test_issame_list)}
    bins_test_names = [bin_loc.rsplit('/')[-2] +'_'+ bin_loc.rsplit('/',1)[-1].split('.')[0] for bin_loc in bins_test_list]
    bins_train_names = [bin_loc.rsplit('/')[-2] +'_'+ bin_loc.rsplit('/',1)[-1].split('.')[0] for bin_loc in bins_train_list]
    all_bins_names_dict = {'train': bins_train_names, 'test': bins_test_names}
...
forward_bins_through_models(MODEL_DIR_LOC, DBS_MODELS_DATA_LOC, IMAGE_SIZE, BATCH_SIZE)
