...
def main(args):
    ...
    for ds_dir in datasets_dirs:
        for masked_dir in os.listdir(ds_dir):
            ...
            if ds_dir.find('/lfw') != -1:
                lfw_dirs.append(os.path.join(ds_dir, masked_dir))
            elif ds_dir.find('/agedb') != -1:
                agedb30_dirs.append(os.path.join(ds_dir, masked_dir))
            elif ds_dir.find('/casia') != -1:
                casia_dirs.append(os.path.join(ds_dir, masked_dir))
            else:
                print(f'What is this dataset?! {ds_dir}')
                # exit(1)
    # lfw_dirs = [dir for dir in masked_faces_dirs if ds_dir.find('/lfw2/') != -1]
    # agedb30_dirs = [dir for dir in masked_faces_dirs if dir.find('/agedb/') != -1]
    # casia_dirs = [dir for dir in masked_faces_dirs if dir.find('/casia/') != -1]
    ...
    # run_multy(make_bin, masked_faces_input_dirs)
