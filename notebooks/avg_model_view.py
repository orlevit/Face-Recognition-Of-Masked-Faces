...
for split in range(1, splits_number + 1):
    ...
    if model_name != NOT_TO_PROCESS:
       ...
       means_arr = np.array([float(i.split(splitter)[0]) for i in selected_split_df['averaged_auc'].tolist()])
       stds_arr = np.array([float(i.split(splitter)[1]) for i in selected_split_df['averaged_auc'].tolist()])
       mean_auc = np.mean(means_arr)
       std_auc = np.sqrt(np.sum(stds_arr**2)/len(stds_arr))
       result = str(np.round(mean_auc, 3)) + splitter + str(np.round(std_auc, 3))
